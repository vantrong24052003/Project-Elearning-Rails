...
class Manage::QuizAttemptsController < Manage::BaseController
  ...
  def show
    ...
    @cheating_stats = {
      tab_switches: cheating_logs.count { |entry| ['tab_switch', 'window_blur'].include?(entry['action']) },
      copy_paste: cheating_logs.count { |entry| ['copy', 'paste', 'cut'].include?(entry['action']) },
      ...
      devtools: cheating_logs.count { |entry| ['devtools_open', 'devtools_key'].include?(entry['action']) },
      other: cheating_logs.count { |entry| !['tab_switch', 'window_blur', 'copy', 'paste', 'cut', 
